...
class Config:
    ...
    def __init__(self):
        ...
        self.SECRETID = get_env_or_raise("SECRETID")
        self.SECRETKEY = get_env_or_raise("SECRETKEY")
        ...
        # 加载自定义IP配置
        self.self_cm_cfips_list = parse_custom_ips(os.getenv("CUSTOM_CM_IPS", ""))
        self.self_cu_cfips_list = parse_custom_ips(os.getenv("CUSTOM_CU_IPS", ""))
        self.self_ct_cfips_list = parse_custom_ips(os.getenv("CUSTOM_CT_IPS", ""))
        self.self_def_cfips_list = parse_custom_ips(os.getenv("CUSTOM_DEF_IPS", ""))
        
        # API优先级配置
        self.api_priority = self._load_api_priority()
    ...
    def _load_domains(self):
        ...
        try:
            ...
            raise ValueError(f"DOMAINS JSON 解析错误: {e}")
    
    def _load_api_priority(self):
        """加载API优先级配置"""
        priority_str = os.getenv("API_PRIORITY", "")
        if not priority_str:
        ...
        try:
            priority_list = [int(x.strip()) for x in priority_str.split(',') if x.strip().isdigit()]
            api_map = {api['id']: api for api in self.API_LIST}
            
            sorted_apis = []
            for priority in priority_list:
                if priority in api_map:
                    api = api_map[priority].copy()
                    api['priority'] = len(sorted_apis) + 1
                    sorted_apis.append(api)
            
            remaining_apis = [api for api in self.API_LIST if api['id'] not in priority_list]
            for api in remaining_apis:
                api_copy = api.copy()
                api_copy['priority'] = len(sorted_apis) + 1
                sorted_apis.append(api_copy)
            ...
            logging.error(f"API优先级配置解析错误: {e}")
            return self.API_LIST
...
class IPCache:
    ...
    def get(self, record_type):
        """获取缓存的IP数据"""
        cache_key = "ipv6" if record_type == "AAAA" else "ipv4"
        return self._cache[cache_key]
    
    def set(self, record_type, data):
        """设置IP缓存数据"""
        cache_key = "ipv6" if record_type == "AAAA" else "ipv4"
...
class IPManager:
    ...
    def get_optimization_ip(self, line_config=None):
        """获取优化后的 IP 地址"""
        record_type = self.config.RECORD_TYPE
        
        # IPv6 只从缓存获取数据
        if record_type == "AAAA":
            return self.cache.get(record_type)
        
        # 检查 IPv4 缓存
        cached_data = self.cache.get(record_type)
        if cached_data:
            return cached_data
        
        # 获取新数据
        for api in self.config.api_priority:
            try:
                api_id = api['id']
                api_priority = api.get('priority', api_id)
                logging.info(f"尝试从 {api['name']} (ID: {api_id}, 优先级: {api_priority}) 获取IP数据")
                ...
                # 根据不同API处理数据
                if api['name'] == 'vvhan':
                    api_result = self._process_vvhan_data(result)
                elif api['name'] == 'hostmonit':
                    api_result = self._process_hostmonit_data(result)
                elif api['name'] == '164746':
                    api_result = self._process_164746_data(result)
                elif api['name'] == 'wetest':
                    api_result = self._process_wetest_data(result)
                else:
                    continue
                
                if api_result:
                    self.cache.set(record_type, api_result)
                    return api_result
    ...
    def _process_vvhan_data(self, result):
        """处理 vvhan API 数据"""
        if not result.get("success") or "data" not in result:
            return None
        
        data = result["data"]
        ipv4_data = data.get("v4", {})
        ipv6_data = data.get("v6", {})
        
        if self.config.RECORD_TYPE == "A":
            return self._process_ip_data(ipv4_data)
        else:
            return self._process_ip_data(ipv6_data)

    def _process_hostmonit_data(self, result):
        """处理 hostmonit API 数据"""
        if result.get("code") != 200:
            return None
        
        # hostmonit 格式：
        # {"code": 200, "data": {"CM": [{"ip": "xx", "ttl": 60, "ping": 123}]}}
        formatted_data = {}
        for isp, ips in result.get("data", {}).items():
            formatted_data[isp] = [{"ip": ip["ip"], "latency": ip.get("ping", 999)} for ip in ips]
        
        return self._process_ip_data(formatted_data)

    def _process_164746_data(self, result):
        """处理 164746 API 数据"""
        ...
        for isp, ips in result.items():
            formatted_data[isp] = [{"ip": ip["ip"], "latency": ip.get("delay", 999)} for ip in ips]
        
        return self._process_ip_data(formatted_data)

    def _process_wetest_data(self, result):
        """处理 wetest API 数据"""
        if result.get("code") != 200:
            return None
        
        # wetest 格式：
        # {"code": 200, "data": {"CM": [{"ip": "xx", "latency": 123}]}}
        formatted_data = {}
        for isp, ips in result.get("data", {}).items():
            formatted_data[isp] = [{"ip": ip["ip"], "latency": ip.get("latency", 999)} for ip in ips]
        
        return self._process_ip_data(formatted_data)

    def _process_ip_data(self, data):
        """处理统一格式的 IP 数据"""
        result = {
            "code": 200,
            "info": {}
        ...
        # 按延迟排序
        sorted_ips = sorted(all_ips, key=lambda x: x.get("latency", float('inf')))
        best_ips = [{"ip": ip["ip"]} for ip in sorted_ips[:self.config.AFFECT_NUM]]
        
        # 为每个运营商准备IP列表
        for line_code in ["CM", "CU", "CT"]:
            if line_code in data:
                line_ips = sorted(data[line_code], key=lambda x: x.get("latency", float('inf')))
                result["info"][line_code] = [{"ip": ip["ip"]} for ip in line_ips[:self.config.AFFECT_NUM]]
...
class DNSManager:
    ...
    def update_records(self, domain, sub_domain, line_config):
        ...
        for line_code in line_config:
            ...
            try:
                self._handle_single_line(domain, sub_domain, line_code, line_name, line_config)
            except Exception as e:
                logging.error(f"{masked_subdomain}.{masked_domain} {line_code}: {str(e)}")
    
    def _get_line_name(self, line_code):
        """获取线路名称"""
        line_info = self.config.ISP_CONFIG.get(line_code)
        return line_info[0] if line_info else None
    
    def _handle_single_line(self, domain, sub_domain, line_code, line_name, line_config):
        """处理单个线路的DNS记录"""
        masked_domain = mask_domain(domain)
        masked_subdomain = mask_domain(sub_domain)
        ...
        if self.config.RECORD_TYPE == "AAAA":
            if current_records:
                logging.info(f"域名 {masked_subdomain}.{masked_domain} 已存在 IPv6 记录，跳过更新")
                return
        
        # 获取优化IP
        has_default_line = "DEF" in line_config
        api_result = self.ip_manager.get_optimization_ip(line_config)
        
        if not api_result:
            if self.config.RECORD_TYPE == "AAAA" and not current_records:
                raise ValueError(f"域名 {masked_subdomain}.{masked_domain} 无 IPv6 记录且无法获取 IPv6 地址")
            return
        
        # 获取当前线路的IP
        if has_default_line or self.config.RECORD_TYPE == "AAAA":
        ...
        # 更新记录
        self._update_dns_record(domain, sub_domain, line_name, new_ips, current_records)
    
    def _update_dns_record(self, domain, sub_domain, line_name, new_ips, current_records):
        """更新DNS记录"""
        masked_domain = mask_domain(domain)
        masked_subdomain = mask_domain(sub_domain)
        ...
        try:
            ...
            raise RuntimeError(f"记录操作失败: {str(e)}")
